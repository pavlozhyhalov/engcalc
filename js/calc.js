...
let _m = { type: '', inputs: [], steps: [], checks: [], resultsHtml: '', schemaHtml: '', schemaParams: {}, motorPm: 0, reducerNout: 0, chainFone: 0, beltF2: 0, beltB: 0, shaftM: 0 };
...
function applyPreset(selId, rhoId) {
  ...
  const mat = MAT_DB[s.value];
  if (!mat) return;
  // Extract prefix: 'b_rho' → 'b_', 'sv_rho' → 'sv_'
  const prefix = rhoId.slice(0, rhoId.lastIndexOf('_') + 1);
  const fill = { phi: mat.phi, psi: mat.psi, Cm: mat.Cm };
  for (const [key, val] of Object.entries(fill)) {
    const el = $id(prefix + key);
...
function showResults(html) {
  ...
  if (box) {
    ...
    _m.resultsHtml = html;
    const svgHtml = (typeof makeSchema === 'function') ? makeSchema(_m.type, _m.schemaParams) : '';
...
<button class="calc-btn" onclick="CALCS.belt.run()">Розрахувати</button>`,
    ...
    run() {
      ...
      _m.schemaParams = { L: num('b_L'), H: num('b_H'), beta: beta };
      showResults(`
...
<button class="calc-btn" onclick="CALCS.screw.run()">Розрахувати</button>`,
    ...
    run() {
      ...
      _m.schemaParams = { D: num('s_D') / 1000, L: num('s_L') };
      showResults(`
...
<button class="calc-btn" onclick="CALCS.mesh_chain.run()">Розрахувати</button>`,
    ...
    run() {
      ...
      _m.schemaParams = { L: num('m_L'), H: num('m_H'), B: num('m_B') };
      showResults(`
...
<button class="calc-btn" onclick="CALCS.plate.run()">Розрахувати</button>`,
    ...
    run() {
      ...
      _m.schemaParams = { L: num('pl_L'), H: num('pl_H'), B: num('pl_B') };
...
${safety < 8 ? `<div class="note warn">Запас міцності ${fmt(safety,1)}× нижче норми 8×. Оберіть ланцюг з більшим розривним навантаженням ≥ ${fmt(Fchain*8/1000,0)} кН.</div>` : `<div class="note ok">Запас міцності ланцюга ${fmt(safety,1)}× — у нормі (≥8×).</div>`}`);
    ...
    example: `<div class="ex-intro">Пластинчастий конвеєр — горизонтальна траса. Вихідні дані: L = 12 м, B = 500 мм, q<sub>ван</sub> = 15 кг/м, v = 0.1 м/с, f = 0.15, η = 0.82, k = 1.4, d<sub>s</sub> = 200 мм, q<sub>t</sub> = 8 кг/м, q<sub>p</sub> = 20 кг/м.</div>
<div class="ex-grid">
  <div class="step"><div class="step-n">1</div><div class="step-body"><strong>Рухома маса q<sub>рух</sub></strong><div class="step-formula">q<sub>рух</sub> = q<sub>t</sub> + q<sub>p</sub> = 8 + 20 = 28 кг/м</div></div></div>
  <div class="step"><div class="step-n">2</div><div class="step-body"><strong>Тертя робочої гілки</strong><div class="step-formula">F<sub>роб</sub> = f · (q<sub>рух</sub> + q<sub>ван</sub>) · g · L = 0.15 · (28 + 15) · 9.81 · 12 = 760 Н</div></div></div>
  <div class="step"><div class="step-n">3</div><div class="step-body"><strong>Тертя холостої гілки</strong><div class="step-formula">F<sub>хол</sub> = f · q<sub>рух</sub> · g · L = 0.15 · 28 · 9.81 · 12 = 494 Н</div></div></div>
  <div class="step"><div class="step-n">4</div><div class="step-body"><strong>Сумарне зусилля (β = 0°, F<sub>v</sub> = 0)</strong><div class="step-formula">F = F<sub>роб</sub> + F<sub>хол</sub> = 760 + 494 = 1254 Н</div></div></div>
  <div class="step"><div class="step-n">5</div><div class="step-body"><strong>Потужність приводу</strong><div class="step-formula">P = F · v / η · k / 1000 = 1254 · 0.1 / 0.82 · 1.4 / 1000 = <b>0.214 кВт → двигун 0.25 кВт</b></div></div></div>
  <div class="step"><div class="step-n">6</div><div class="step-body"><strong>Момент та частота зірочки</strong><div class="step-formula">M = F · d<sub>s</sub>/2 = 1254 · 0.1 = 125.4 Н·м &nbsp;|&nbsp; n = 60 · v / (π · d<sub>s</sub>) = 60 · 0.1 / (π · 0.2) = 9.5 об/хв</div></div></div>
  <div class="step"><div class="step-n">7</div><div class="step-body"><strong>Натяг на один ланцюг та запас міцності</strong><div class="step-formula">F<sub>лан</sub> = F / 2 = 627 Н &nbsp;|&nbsp; Запас = 28 500 / 627 = 45× ✓ (ланцюг 28A-1, F<sub>розр</sub> = 28.5 кН)</div></div></div>
</div>`
  },
...
<button class="calc-btn" onclick="CALCS.chain_scraper.run()">Розрахувати</button>`,
    run() {
      ...
      _m.schemaParams = { L: num('c_L'), H: num('c_H') };
      showResults(`
<div class="res-hl"><span class="big">${fmt(Pm,2)} кВт</span><span class="lbl">Потужність приводу · запас ×${k}</span></div>
<div class="rgroup"><div class="rgroup-t">Продуктивність</div>
...
<button class="calc-btn" onclick="CALCS.roller.run()">Розрахувати</button>`,
    run() {
      ...
      _m.schemaParams = { L: num('r_L'), beta: num('r_beta') || 0, m: fmt(num('r_m') || 0, 0) };
      showResults(`
...
<button class="calc-btn" onclick="CALCS.drag.run()">Розрахувати</button>`,
    ...
    run() {
      ...
      _m.schemaParams = { L: num('d_L'), H: num('d_H') };
...
${safety < 6 ? `<div class="note warn">Запас міцності ${fmt(safety,1)}× нижче норми 6× (CEMA 550). Оберіть ланцюг F_розр ≥ ${fmt(Ftot*6/1000,0)} кН.</div>` : `<div class="note ok">Запас міцності ланцюга ${fmt(safety,1)}× — у нормі (≥6×).</div>`}`);
    ...
    example: `<div class="ex-intro">Волочильний (En-masse) конвеєр — зерно (ρ = 0.75 т/м³). Вихідні дані: L = 20 м, W = 300 мм, h = 200 мм, v = 0.25 м/с, f₁ = 0.35 (ланцюг), f₂ = 0.50 (матеріал), η = 0.82, k = 1.4, d<sub>s</sub> = 250 мм, q<sub>c</sub> = 5 кг/м, F<sub>розр</sub> = 100 кН.</div>
<div class="ex-grid">
  <div class="step"><div class="step-n">1</div><div class="step-body"><strong>Погонна маса матеріалу q<sub>m</sub></strong><div class="step-formula">q<sub>m</sub> = W · h · ρ · 1000 = 0.3 · 0.2 · 0.75 · 1000 = 45 кг/м</div><div class="step-result">Q = 45 · 0.25 · 3.6 = 40.5 т/год</div></div></div>
  <div class="step"><div class="step-n">2</div><div class="step-body"><strong>Тертя матеріалу по дну жолоба</strong><div class="step-formula">F<sub>мат</sub> = f₂ · q<sub>m</sub> · g · L = 0.50 · 45 · 9.81 · 20 = 4 415 Н</div></div></div>
  <div class="step"><div class="step-n">3</div><div class="step-body"><strong>Тертя ланцюга (обидві гілки)</strong><div class="step-formula">F<sub>лан</sub> = 2 · f₁ · q<sub>c</sub> · g · L = 2 · 0.35 · 5 · 9.81 · 20 = 686 Н</div></div></div>
  <div class="step"><div class="step-n">4</div><div class="step-body"><strong>Сумарне зусилля (β = 0°, F<sub>v</sub> = 0)</strong><div class="step-formula">F = F<sub>мат</sub> + F<sub>лан</sub> = 4 415 + 686 = <b>5 101 Н</b></div></div></div>
  <div class="step"><div class="step-n">5</div><div class="step-body"><strong>Потужність приводу</strong><div class="step-formula">P = F · v / η · k / 1000 = 5 101 · 0.25 / 0.82 · 1.4 / 1000 = <b>2.18 кВт → двигун 2.2 кВт</b></div></div></div>
  <div class="step"><div class="step-n">6</div><div class="step-body"><strong>Момент та частота зірочки</strong><div class="step-formula">M = 5 101 · 0.125 = 638 Н·м &nbsp;|&nbsp; n = 60 · 0.25 / (π · 0.25) = 19.1 об/хв</div></div></div>
  <div class="step"><div class="step-n">7</div><div class="step-body"><strong>Запас міцності ланцюга</strong><div class="step-formula">Запас = 100 000 / 5 101 = 19.6× > 6× (CEMA 550) ✓</div></div></div>
</div>`
  },
...
<button class="calc-btn" onclick="CALCS.screw_vertical.run()">Розрахувати</button>`,
    ...
    run() {
      ...
      _m.motorPm = Pt;
      _m.reducerNout = n; _m.chainFone = 0; _m.beltF2 = 0; _m.shaftM = M;
      _m.schemaParams = { H: num('sv_H'), D: num('sv_D') / 1000 };
...
${n > nCr*0.6 ? `<div class="note warn">Частота ${fmt(n,1)} об/хв перевищує 60% критичної (${fmt(nCr*0.6,0)} об/хв). Матеріал прилипне до стінок жолоба — збільшіть D або зменшіть крок t.</div>` : `<div class="note ok">Частота ${fmt(n,1)} об/хв = ${fmt(n/nCr*100,0)}% від критичної — у нормі.</div>`}`);
    ...
    example: `<div class="ex-intro">Вертикальний шнек — зерно (ρ = 0.75 т/м³, ψ = 0.25). Вихідні дані: D = 200 мм, t = 200 мм, H = 4 м, Q = 10 т/год, C<sub>m</sub> = 2.0, η = 0.80, k = 1.5.</div>
<div class="ex-grid">
  <div class="step"><div class="step-n">1</div><div class="step-body"><strong>Потрібна частота обертання</strong><div class="step-formula">n = Q / (47.1 · D² · t · ρ · ψ) = 10 / (47.1 · 0.04 · 0.2 · 0.75 · 0.25) = 10 / 0.0707 = <b>141 об/хв</b></div></div></div>
  <div class="step"><div class="step-n">2</div><div class="step-body"><strong>Критична частота обертання</strong><div class="step-formula">n<sub>кр</sub> = 60 / (2π) · √(g / (0.057 · D)) = 9.55 · √(9.81 / 0.0114) = 9.55 · 29.3 = <b>280 об/хв</b></div><div class="step-result">n / n<sub>кр</sub> = 141 / 280 = 50% — у нормі (≤ 60%) ✓</div></div></div>
  <div class="step"><div class="step-n">3</div><div class="step-body"><strong>Потужність приводу (C<sub>m,верт</sub> = C<sub>m</sub> + 1.5)</strong><div class="step-formula">P = Q · H · (C<sub>m</sub> + 1.5) · k / (367 · η) = 10 · 4 · 3.5 · 1.5 / (367 · 0.80) = 210 / 293.6 = <b>0.72 кВт → двигун 0.75 кВт</b></div></div></div>
  <div class="step"><div class="step-n">4</div><div class="step-body"><strong>Крутний момент на валу</strong><div class="step-formula">M = P · 1000 / (2π · n / 60) = 720 / (2π · 141 / 60) = 720 / 14.76 = <b>48.8 Н·м</b></div></div></div>
</div>
<div class="note info" style="margin-top:8px">При Q = 30 т/год n = 424 об/хв > 60% · n<sub>кр</sub> = 168 об/хв — матеріал центрифугуватиме. Збільшіть D до Ø315 мм або зменшіть Q / збільшіть крок t.</div>`
  },
